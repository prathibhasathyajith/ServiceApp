...
public class ServiceRequestDAO {
    ...
    public void getChartDataSummary2(ServiceRequestInputBean inputBean) throws Exception {
        ...
        try {
            ...
            if (ObjetctListCount.size() > 0) {
                ...
                sqlSearch = "SELECT "
                        + "YEAR ( s.updated_time ) as years , " //0
                        + "MONTH ( s.updated_time ) as months , " //1
                        + "SUM( ss.status_code = 'COMPLETED' ) AS completedReq ,  "//2
                        + "SUM( ss.status_code = 'C_CNCLED' ) AS cusCancelReq , "//3
                        + "SUM( ss.status_code = 'B_CNCLED' ) AS bassCancelReq , "//4
                        + "SUM( ss.status_code = 'C_REJCT' ) AS cusRejReq , "//5
                        + "SUM( ss.status_code = 'B_REJCT' ) AS bassRejReq , "//6
                        + "SUM( ss.status_code = 'PUSHED_B' ) AS bassPushedReq , "//7
                        + "SUM( ss.status_code = 'INIT' ) AS initReq , "//8
                        + "COUNT( * ) AS totalReq ,  "//9
                        + "SUM( ss.status_code = 'C_CNCLED' || ss.status_code = 'B_CNCLED') AS cancelAll , "//10
                        + "SUM( ss.status_code = 'B_REJCT' || ss.status_code = 'C_REJCT' ) AS rejAll "//11
                        + "FROM mob_service_request AS s "
                        + "LEFT OUTER JOIN `status` AS ss ON s.`status` = ss.status_code "
                        + "WHERE s.updated_time <= '" + edat + "' AND s.updated_time > '" + sdat + "' "
                        + "GROUP BY YEAR ( s.updated_time ), MONTH ( s.updated_time )  "
                        + "ORDER BY MONTH ( s.updated_time )  ASC ";
                ...
                for (Object[] bean : ObjetctList) {
                    ...
                    if (bean[0] != null) {
                        map.setYear(bean[0].toString());
                    } else {
                        map.setYear("--");
                    }

                    if (bean[1] != null) {
                        map.setMonth(bean[1].toString());
                        map.setMonthDes(this.monthDescription(Integer.parseInt(bean[1].toString())));
                    } else {
                        map.setMonth("--");
                        map.setMonthDes("--");
                    }
                    if (bean[2] != null) {
                        map.setCompletedReq(bean[2].toString());
                    } else {
                        map.setCompletedReq("--");
                    }

                    if (bean[3] != null) {
                        map.setCusCancelReq(bean[3].toString());
                    } else {
                        map.setCusCancelReq("--");
                    }

                    if (bean[4] != null) {
                        map.setBassCancelReq(bean[4].toString());
                    } else {
                        map.setBassCancelReq("--");
                    }

                    if (bean[5] != null) {
                        map.setCusRejReq(bean[6].toString());
                    } else {
                        map.setCusRejReq("--");
                    }

                    if (bean[6] != null) {
                        map.setBassRejReq(bean[6].toString());
                    } else {
                        map.setBassRejReq("--");
                    }

                    if (bean[7] != null) {
                        map.setBassPushedReq(bean[7].toString());
                    } else {
                        map.setBassPushedReq("--");
                    }

                    if (bean[8] != null) {
                        map.setInitReq(bean[8].toString());
                    } else {
                        map.setInitReq("--");
                    }

                    if (bean[9] != null) {
                        map.setTotalReq(bean[9].toString());
                    } else {
                        map.setTotalReq("--");
                    }
                    
                    if (bean[10] != null) {
                        map.setCancelAll(bean[10].toString());
                    } else {
                        map.setCancelAll("--");
                    }

                    if (bean[11] != null) {
                        map.setRejAll(bean[11].toString());
                    } else {
                        map.setRejAll("--");
                    }
